# Finance — Terms of Service

**Effective Date:** 2026-06-01

> This document may be updated as Finance evolves. Check back for the latest
> version at <https://finance.jrmoulckers.com/terms>.

## 1. Acceptance of Terms

These Terms of Service ("Terms") govern your use of Finance, a local-first personal financial tracking application developed and operated by J.R. Moulckers ("the developer," "I," "me," or "my"). By downloading, accessing, or using Finance, you agree to these Terms. If you do not agree, do not use Finance.

If you use Finance on behalf of a company, household, or other organization, you represent that you have authority to bind that entity to these Terms.

## 2. Description of Service

Finance is a multi-platform, local-first personal financial tracking application. It helps users record and review accounts, balances, transactions, budgets, goals, categories, and related financial information across iOS, Android, Windows, and web experiences.

Finance is designed to work primarily on your device. Core tracking features can be used without cloud sync. Optional account-based features may include multi-device sync, household sharing, and premium features.

## 3. User Accounts

You may use certain parts of Finance without creating an account. If you choose to create an account, you agree to provide accurate information and to keep it up to date.

You are responsible for:

- maintaining the confidentiality of your login credentials, passkeys, recovery methods, and devices;
- all activity that occurs under your account; and
- notifying me promptly at support@jrmoulckers.com if you suspect unauthorized access to your account.

You must be at least 13 years old, or the minimum age required in your jurisdiction, to create and use an account.

## 4. Acceptable Use

You agree not to use Finance to:

- violate any law, regulation, or third-party right;
- upload, store, or transmit unlawful, fraudulent, abusive, or misleading content;
- attempt to gain unauthorized access to the service or related systems;
- interfere with the integrity, performance, or security of the service;
- reverse engineer, decompile, disassemble, or otherwise attempt to derive source code or underlying technology except where applicable law or the applicable software license expressly permits it; or
- use Finance in connection with illegal financial activity, including fraud, money laundering, sanctions evasion, or terrorist financing.

We may suspend or limit access to accounts that violate these Terms.

## 5. User Content and Data

You retain ownership of the financial data and other content you create, upload, or store in Finance ("User Content"). We do not claim ownership of your transactions, balances, budgets, goals, categories, or related personal financial records.

You grant us a limited right to host, process, transmit, back up, and display User Content only as needed to operate the service features you choose to use, such as synchronization, account security, export, support, and household sharing.

If you enable optional sync or shared features:

- your data may be transmitted to and stored with infrastructure providers used to operate the service;
- authorized household members or collaborators may be able to view shared data you choose to share; and
- you remain responsible for backing up or exporting your data before deleting your account or ending use of the service.

Finance provides export tools intended to let you retrieve your data in machine-readable formats. Availability and scope of export features may evolve over time.

## 6. Free and Premium Tiers

Finance uses a freemium model.

### Free Tier

The free tier is intended to provide a complete single-device personal finance tracker, including core local-first budgeting and transaction tracking features.

### Premium Tier

Premium may include features such as:

- multi-device sync;
- household or partner sharing;
- AI-assisted categorization, forecasting, and insights;
- advanced reports or custom visualizations; and
- premium support or other paid benefits.

We may change, add, or retire features in either tier from time to time. Material pricing or subscription changes will be disclosed before they take effect.

## 7. Subscriptions and Billing

Premium features may require a recurring subscription or other paid plan. Pricing, billing intervals, trial terms, and included features will be shown in the app, website, or app store listing before purchase.

Unless otherwise stated:

- subscriptions renew automatically until canceled;
- you may cancel at any time through the app store or payment provider used for purchase;
- cancellation stops future renewals but generally does not retroactively refund the current billing period; and
- refunds, credits, and billing disputes are handled according to the rules of the applicable app store, payment provider, and governing law.

Finance is not a payment processor and does not directly process card payments or hold customer deposits.

## 8. Privacy

Your use of Finance is also subject to our Privacy Policy, which explains what data we collect, how we use it, how we protect it, and what choices you have. Optional analytics, diagnostics, or crash reporting features, if enabled, are intended to be consent-based.

Please review the Privacy Policy at <https://finance.jrmoulckers.com/privacy>.

## 9. Disclaimers

Finance is a financial tracking tool only. **Finance is currently in alpha and
is provided on an "as-is" and "as-available" basis.**

- Finance is **not financial, tax, legal, accounting, or investment advice**.
- Finance is **not a bank, lender, broker, insurer, or payment processor**.
- Finance does **not** guarantee that any calculation, summary, forecast, categorization, or recommendation is accurate, complete, or suitable for your situation.
- AI-assisted features may generate inaccurate, incomplete, delayed, or inappropriate outputs.
- Finance may be unavailable, interrupted, or contain errors from time to time.

You are solely responsible for verifying your records and for any financial decisions you make.

## 10. Limitation of Liability

To the fullest extent permitted by law, J.R. Moulckers and any contributors, contractors, or licensors will not be liable for any indirect, incidental, special, consequential, exemplary, or punitive damages, or for any loss of profits, revenues, data, goodwill, or business opportunities arising out of or related to your use of the service.

To the fullest extent permitted by law, total liability for claims arising out of or related to these Terms or the service will not exceed the amount you paid for Finance in the 12 months before the event giving rise to the claim, or USD $50 if you have not made any payment.

Some jurisdictions do not allow certain limitations of liability, so some of the limitations above may not apply to you.

## 11. Termination

You may stop using Finance at any time. If you have an account, you may request account deletion or use any self-service deletion tools we make available.

Before terminating your account, you should export any data you want to keep. After deletion or termination, some data may remain for a limited period where required for security, fraud prevention, legal compliance, dispute resolution, backup integrity, or shared-data handling. Where household or shared data is involved, some records may remain visible to other authorized users depending on the sharing model in effect.

We may suspend or terminate your access if you violate these Terms, create risk for other users or the service, or if required to do so by law.

## 12. Alpha and Pre-Release Terms

Finance is currently in **alpha**. During this phase:

- features may change, break, or be removed without advance notice;
- data loss is possible — you should maintain your own backups;
- service availability is not guaranteed; and
- the developer may reset accounts or databases during development.

Your use of the alpha constitutes acceptance of these additional risks.

## 13. Changes to Terms

We may update these Terms from time to time. If we make material changes, we will post the updated Terms, update the effective date, and provide reasonable notice where required by law.

Your continued use of Finance after updated Terms take effect means you accept the revised Terms.

## 14. Governing Law

<!-- TODO: confirm jurisdiction before GA -->

These Terms are governed by applicable law in the jurisdiction where the developer resides. Any dispute arising from these Terms or your use of Finance will be resolved in accordance with applicable law.

## 15. Contact

If you have questions about these Terms, please contact:

- **Developer:** J.R. Moulckers
- **Email:** support@jrmoulckers.com <!-- TODO: confirm support email -->
- **Web:** <https://finance.jrmoulckers.com/support>

---

_Last updated: 2026-05-13 · Version 1.0 (Alpha)_
